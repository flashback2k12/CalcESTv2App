...
public class AuswertungEV extends Activity {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.feedback:
			...
			Email.putExtra(Intent.EXTRA_EMAIL,
					new String[] { "feedbackcalcest@gmail.com" });
			Email.putExtra(Intent.EXTRA_SUBJECT, "Feedback");
			Email.putExtra(
					Intent.EXTRA_TEXT,
					"Vielen Dank für das testen meiner ersten App! Bitte sende mir deine Meinung! Angaben: Telefon/Android-OS/Pro/Kon/Anregungen! Danke! :-)");
		...
		case R.id.credits:
			...
			tv1.setText(Html
					.fromHtml("<a href=https://github.com/flashback2k12/CalcESTv2App> GITHUB "));
			tv1.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv2 = (TextView) findViewById(R.id.textView3);
			tv2.setText(Html
					.fromHtml("<a href=http://stackoverflow.com/questions/4930228/open-a-url-on-click-of-ok-button-in-android> open url with button click "));
			tv2.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv3 = (TextView) findViewById(R.id.textView4);
			tv3.setText(Html
					.fromHtml("<a href=http://stackoverflow.com/questions/9852184/android-textview-hyperlinkp> textview hyperlink "));
			tv3.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv4 = (TextView) findViewById(R.id.textView5);
			tv4.setText(Html
					.fromHtml("<a href=http://stackoverflow.com/questions/11320685/how-to-implement-send-feedback-feature-in-android> send feedback "));
			tv4.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv5 = (TextView) findViewById(R.id.textView6);
			tv5.setText(Html
					.fromHtml("<a href=http://www.java-samples.com/showtutorial.php?tutorialid=1523> make folder / file "));
			tv5.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv6 = (TextView) findViewById(R.id.textView7);
			tv6.setText(Html
					.fromHtml("<a href=http://www.vogella.com/blog/2011/07/19/android-shapes/> background.xml "));
			tv6.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv7 = (TextView) findViewById(R.id.textView8);
			tv7.setText(Html
					.fromHtml("<a href=http://www.androidaspect.com/2012/12/android-splash-screen-tutorial.html> splashScreen "));
			tv7.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv8 = (TextView) findViewById(R.id.textView9);
			tv8.setText(Html
					.fromHtml("<a href=http://androidgps.blogspot.de/2008/09/writing-to-sd-card-in-android.html> Filewriter "));
			tv8.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv9 = (TextView) findViewById(R.id.textView10);
			tv9.setText(Html
					.fromHtml("<a href=http://stackoverflow.com/questions/3993924/get-android-api-level-of-phone-currently-running-my-application> Check OS VersionsCode "));
			tv9.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv10 = (TextView) findViewById(R.id.textView11);
			tv10.setText(Html
					.fromHtml("<a href=http://stackoverflow.com/questions/3093365/how-can-i-check-the-system-version-of-android> Check OS VersionsCode "));
			tv10.setMovementMethod(LinkMovementMethod.getInstance());
			TextView tv11 = (TextView) findViewById(R.id.textView13);
			tv11.setText(Html
					.fromHtml("<a href=http://androidcodesnips.blogspot.de/2011/10/popupwindow-in-android.html> Popup-Window "));
	...
	public void BackToAuswertung(View view){
		setContentView(R.layout.activity_ev_auswertung);
	}

	public void btnAuswertung(View view) {
		/*
		 * Variablen aus Userdaten deklarieren, Umwandlung von Strings in
		 * numaerische Werte
		 */
		UserdatenEV user = (UserdatenEV) getApplication();
		int mSteuerJahr = Integer.parseInt(user.getSteuerJahr());
		double mBruttoGehaltMonat = Double.parseDouble(user
				.getBruttoGehaltMonat());
		...
		double mArbeitsmittelGezahlt = Double.parseDouble(user
				.getArbeitsmittelGezahlt());
		double mTelefonkostenGezahlt = Double.parseDouble(user
		...
		// double mRiesterV = Double.parseDouble(user.getRieserV()); -->
		// Funktionalitaet noch nicht eingebaut
		double mLvMitKap = Double.parseDouble(user.getLvMitKap());
		double mLvOhneKap = Double.parseDouble(user.getLvOhneKap());
		double mKrankheitsKostenGezahlt = Double.parseDouble(user
				.getKrankheitsKostenGezahlt());
		double mHhMitMinijob = Double.parseDouble(user
				.getHaushaltshilfeMitMinijob());
		double mHhOhneMinijob = Double.parseDouble(user
				.getHaushaltshilfeOhneMinijob());
		 ...
		 */
		mJahresBruttoGehalt = BerechneEV.JahresWerte(mArbeitsMonate,
				mBruttoGehaltMonat);
		mJahresLohnSteuer = BerechneEV.JahresWerte(mArbeitsMonate,
				mLohnSteuerMonat);
		 ...
		 */
		mWerbungsKosten = BerechneEV.WerbungsKosten(mSteuerJahr, mArbeitsTage,
				mEntfernungWA, mArbeitsmittelGezahlt, mTelefonkostenGezahlt);
		/*
		 * Berechnung Summe der Einkuenfte
		 */
		mSummeEinkunft = BerechneEV.SummeEinkunft(mJahresBruttoGehalt,
				mWerbungsKosten);
		/*
		 ...
		 */
		mSpendenAbzug = BerechneEV.SpendenAbzug(mGesamtbetragEinkunft,
				mSpendenGezahlt);
		/*
		 * Berechnung Abzug der aussergewoehnlichen Belastung
		 */
		mAgBelastungAbzug = BerechneEV.agBelastungAbzug(mGesamtbetragEinkunft,
				mKrankheitsKostenGezahlt);
		/*
		 * Berechnung des Vorsorgeaufwands
		 */
		mVorsorgeAufwand = BerechneEV.vorsorgeAufwand(mSteuerJahr, mJahresRv,
				mJahresKv, mJahresPv, mJahresAv, mHapfpflV, mUnfallV, mBuV,
				mRuerupV, mLvMitKap, mLvOhneKap);
		/*
		 * Berechnung zu versteuerndes Einkommen
		 */
		mZuVerstEinkommen = BerechneEV.zuVerstEinkommen(mGesamtbetragEinkunft,
				mVorsorgeAufwand, mSpendenAbzug, mAgBelastungAbzug);
		/*
		 * Berechnung der Einkommensteuer
		 */
		mEinkommenSteuer = BerechneEV.einkommenSteuer(mSteuerJahr,mZuVerstEinkommen);
		/*
		 * Berechnung des Soli-Zuschlags
		 */
		mSoliZuschlag = BerechneEV.soliZuschlag(mZuVerstEinkommen,
				mKapitalSteuer, mEinkommenSteuer);
		/*
		 * Berechnung Steuerabzug gemaess Paragraph 35a EStG
		 */
		mHnDlAbzug = BerechneEV.hnDlAbzug(mHhMitMinijob, mHhOhneMinijob,
				mHandwerkerL);
		/*
		 * Berechnung des Ergebnis
		 */
		mErgebnisBerechnung = BerechneEV.ergebnisSteuer(mEinkommenSteuer,
				mSoliZuschlag, mJahresLohnSteuer, mJahresSolZ, mHnDlAbzug);
		/*
	...
	public void btnCreateFile(View view) {
		 ...
		 */
		UserdatenEV user = (UserdatenEV) getApplication();
		String mSteuerJahr = user.getSteuerJahr();
		 ...
		 */
		AuswertungsdateiEV.erstelle(mSteuerJahr, ml01, ml02, ml03, ml04, ml05,
				ml06, ml07, ml08, ml09, ml010, ml011, ml012, ml013, ml014);
		/*
		 * Zeigen eines Popup-Window, wenn Erstellung beendet wurde
		 */
//		LayoutInflater inflater = (LayoutInflater)this.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
//		PopupWindow pw = new PopupWindow(inflater.inflate(R.layout.activity_base_popup, null, false),100,100,true);
//		pw.showAtLocation(this.getCurrentFocus(), Gravity.CENTER, 0, 0);
		
		View popUpView = getLayoutInflater().inflate(R.layout.activity_base_popup, null);
